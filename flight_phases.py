...
def climb_velocity(aircraft: Aircraft, autopilot: Autopilot) -> bool:
    ...
    if (
        ...
        and not autopilot.SpdMode
        and not in_range(
            aircraft.ias,
            target_speed := knot2ms(aircraft.airplane.climb_v2),
            knot2ms(5),
        )
    ):
        ...
        if in_range(aircraft.ias, target_speed, knot2ms(5)) and aircraft.spd_change < 1:
            if aircraft.n1_target > 0.9:
                ...
                autopilot.Throttle = autopilot.Throttle - 0.1
    elif autopilot.SpdMode and in_range(
        aircraft.mach, (target_speed := aircraft.airplane.climb_v3), 0.01
    ):
...
def cruise(aircraft: Aircraft, autopilot: Autopilot, fpl: IFFPL) -> None:
    ...
    while waypoint != dummy_fix:
        ...
        logger.info(
            f"Next waypoint: {waypoint.name} in {format_time(aircraft.dist_to_next/aircraft.gs)}"
        )
        ...
        if ete_fix <= 0:
            logger.info(
                f"new Altitude: {m2ft(waypoint.altitude):.0f} Vs: {m2ft(target_vs):.0f} ETE: {format_time(ete_fix)}"
            )
            autopilot.Alt = waypoint.altitude
...
def descending(aircraft: Aircraft, autopilot: Autopilot, fpl: IFFPL):
    ...
    while waypoint != dummy_fix:
        ...
        descend_angle = (
            radians(-3)
            if aircraft.next_index <= first_waypoint_index
            else arctan2(delta_alt, aircraft.dist_to_next)
        )
